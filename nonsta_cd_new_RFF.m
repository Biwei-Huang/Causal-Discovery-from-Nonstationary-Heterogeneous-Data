function [gns,g, SP, Yg_save, Yl_save] = nonsta_cd_new_RFF(X,cond_ind_test,maxFanIn,alpha)
...
%%
% recover Markov equivalence class and detect changing causal modules (with PC search)

% Note: t is an surrogate variables to capture nonstationarity; one may replace t with others,
...
%% generalization of invariance to infer causal directions
...
%% independent change principle to infer directions
...
%% nonstationary driving force estimation
...
for i = 1:length(cmodule_id)
    ...
    [Yg,Yl,Mg,Ml,D,eigValueg,eigValuel] = cd_non_con_fun(X(:,pa_id),X(:,x_id),.5,.5);
    Yg_save{i} = Yg;
    Yl_save{i} = Yl;
